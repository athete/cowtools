...
def GetCondorClient(
    ...
    if x509_path is not None:
        ...
        job_script_prologue.append(
            f"export X509_USER_PROXY={os.path.basename(x509_path)}"
        )
    if ship_env:
    ...
    if job_extra_directives["transfer_input_files"] == []:
        ...
        job_extra_directives["transfer_input_files"] = ",".join(
            job_extra_directives["transfer_input_files"]
        )
    if (job_extra_directives["request_GPUs"] is None) or (
        int(job_extra_directives["request_GPUs"]) < 1
    ):
    ...
    print(f"Condor logs, output files, error files in {initial_dir}")
    ...
    if container_info_file.is_file():
        ...
        for dir in cvmfsdirs:
            if os.path.isdir(dir):
                ...
                print_debug(
                    f"Directory '{dir}' does not exist, so leaving as {best_loc}"
                )
